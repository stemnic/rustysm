...
fn read_history_file(file_path : &path::Path, num_lines_to_read: u64, line_offset_from_tail: u64, history_entries: Arc<Mutex<Vec<HistoryLogEntry>>>) -> Result<()> {
...
impl HistoryWatcher {
    pub fn new(history_file: path::PathBuf, lines_to_read: u64, line_offset: u64) -> Result<Self>{
        ...
        read_history_file(&path::PathBuf::from(stru.sm_history_file.to_str().unwrap()), stru.lines_to_read, stru.line_offset, stru.entries.clone());
        return Ok(stru);
    }
    pub fn start(&mut self) {
        ...
        thread::spawn(move || { watch_history_file(history_file_copy, lines_to_read, line_offset, guarded_history_entries, history_update_tx)}); 
...
fn watch_history_file(file_path_buf : path::PathBuf, num_lines_to_read: u64, line_offset_from_tail: u64, history_entries: Arc<Mutex<Vec<HistoryLogEntry>>>, update_notifier: Sender<()>){
    ...
    match read_history_file(file_path, num_lines_to_read, line_offset_from_tail, history_entries.clone()) {Ok(_)=>{}, Err(error)=>warn!("{}",error)};
           ...
           Ok(RawEvent{path: Some(path), op: Ok(op), cookie}) => {
                ...
                let result = match op {
                    notify::op::WRITE | notify::op::CREATE => {
                        let result = read_history_file(file_path, num_lines_to_read, line_offset_from_tail, history_entries.clone());
