...
impl TabsElements {
    ...
    pub fn pos_up(&mut self) -> () {
        ...
        debug!(
            "Setting {} table_list_pos to {}",
            self.display_name, self.table_list_pos
        );
    }
    ...
    pub fn pos_jump_down(&mut self, num: usize) -> () {
        if self.table_list_pos < self.table_list_size
            && self.table_list_pos < self.table_list_size.saturating_sub(num)
        {
    ...
    pub fn update_size(&mut self, size: usize) -> () {
        ...
        trace!(
            "Setting {} table_list_size to {}",
            self.display_name,
            self.table_list_size
        );
    }
}
