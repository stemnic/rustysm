...
impl HistoryLog {
    ...
    pub fn read(&mut self, num_lines_to_read: u64, line_offset_from_tail: u64) -> Result<()> {
        ...
        while let Some(line) = reader.prev_line()? {
            ...
            entries.push(HistoryLogEntry{ unix_timestamp : unix_timestamp_sec, name: types[1].to_string(), location: types[2].to_string()});
...
fn watch_history_file(file_path_buf : path::PathBuf, status_info: Arc<Mutex<QueueInfo>>){
    // Create a channel to receive the events.
    let (tx, rx) = channel();
    ...
    loop {
        match rx.recv() {
            Ok(RawEvent{path: Some(path), op: Ok(op), cookie}) => {
                ...
                if let Err(()) = result {
                    println!("Got error trying to read file: "); // #TODO: Store in struct (pass to terminal_ui)
                }
            },
            Ok(event) => println!("broken event: {:?}", event),
            Err(e) => println!("watch error: {:?}", e),
